...
func (db *PsqlConnection) GetAll() ([]model.Entity, error) {
	rows, err := db.pool.Query(context.Background(), "SELECT id, name, age, ishealthy FROM goschema.newtable")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *PsqlConnection) Delete(uuidString string) error {
	...
	bd, err := db.pool.Exec(context.Background(), "DELETE FROM goschema.newtable WHERE id=$1", &entity.ID)
...
func (db *PsqlConnection) Insert(entity *model.Entity) error {
	entity.ID = uuid.New()
	bd, err := db.pool.Exec(context.Background(), "INSERT INTO goschema.newtable (id, name, age, ishealthy) VALUES($1,$2,$3,$4)", &entity.ID, &entity.Name, &entity.Age, &entity.IsHealthy)
...
func (db *PsqlConnection) Update(uuidString string, entity *model.Entity) error {
	...
	entity.ID = parsedUUID
	bd, err := db.pool.Exec(context.Background(), "UPDATE goschema.newtable SET name=$1, age=$2, ishealthy=$3 WHERE id=$4", &entity.Name, &entity.Age, &entity.IsHealthy, &entity.ID)
